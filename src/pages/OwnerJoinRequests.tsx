...
const OwnerJoinRequests: React.FC = () => {
  ...
  const fetchRequests = async (page = 1) => {
    ...
    try {
      ...
      const filteredRequests = (data.requests || []).filter(
        (r: any) => r.userId !== loggedInUserId,
      );
      const ownerRequests = (data.requests || []).filter(
        (r: any) => r.userId === loggedInUserId,
      );
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      setActionError(
        err.response?.data?.message || `Failed to ${modalAction} request`,
      );
    } finally {
  ...
  const getRoleBadge = (roles?: string[]) =>
    (roles || []).map((r) => (
      <span
        key={r}
        className="px-2.5 py-0.5 bg-indigo-50 text-indigo-700 rounded-lg text-[10px] font-black uppercase tracking-wider border border-indigo-100/50 shadow-sm"
      >
        {r.replace("_", " ")}
      </span>
    ));
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs font-black uppercase tracking-wider ${cfg.badge}`}
      >
        {cfg.icon} {cfg.label}
      </span>
    );
  ...
  const filterTabs: {
  ...
  }[] = [
    {
      ...
      count: summary.total,
      color: "from-slate-700 to-slate-900 shadow-slate-200",
    },
    {
      key: "pending",
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-6xl mx-auto relative">
      ...
      <div className="border-b border-slate-100 pb-2">
        ...
        <p className="text-slate-500 mt-2.5 text-base font-medium">
          Review and manage join requests sent by practitioners targeting your
          clinics
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        {filterTabs.map(({ key, label, count, color }) => (
          <button
            ...
            className={`bg-white rounded-[2.5rem] p-6 shadow-xl transition-all duration-300 text-left border-2 flex items-center justify-between cursor-pointer ${
              filter === key
                ? "border-emerald-500 scale-102 shadow-emerald-50/50"
          ...
          >
            ...
            <div
              className={`p-4 bg-gradient-to-br ${color} rounded-2xl text-white shadow-lg shrink-0`}
            >
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-[2.5rem] p-16 text-center border border-slate-100 shadow-xl shadow-slate-100/35">
          ...
          <p className="text-slate-700 text-lg font-black tracking-tight">
            No affiliation requests found
          </p>
          <p className="text-slate-400 text-sm mt-1.5 max-w-sm mx-auto font-medium">
            {filter === "all"
              ? "No practitioners have submitted affiliation requests to join this clinic yet"
              : `No ${filter} affiliation join records located`}
          </p>
        </div>
      ) : (
              ...
              <div className="flex-1 min-w-0 space-y-5">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="text-2xl font-black text-slate-800 tracking-tight leading-none truncate">
                        {req.fullName || "Practitioner User"}
                      </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-1">
                  ...
                  <div className="p-4 bg-slate-50/50 rounded-2xl border border-slate-100 flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-slate-700 text-xs font-bold mt-0.5 truncate">
                        {req.addedAt
                          ? new Date(req.addedAt).toLocaleDateString("en-IN", {
                              day: "numeric",
                              month: "short",
                              year: "numeric",
                            })
                ...
                {req.rejectionReason && (
                  <div className="p-3.5 bg-rose-50 border border-rose-100 rounded-2xl flex items-start gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-black uppercase text-rose-800 tracking-wider">
                        Reason for Rejection
                      </p>
                      <p className="text-rose-700 text-xs font-semibold mt-1 leading-relaxed">
                        {req.rejectionReason}
                      </p>
                    </div>
                  </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl border border-slate-100 relative overflow-hidden animate-scaleUp">
            ...
            <p className="text-slate-500 text-sm font-semibold mb-6">
              ...
              {modalAction === "approve"
                ? "authorization of "
                : "rejection of "}
              <strong className="text-slate-800">{selectedReq.fullName}</strong>{" "}
              to join practice{" "}
              <strong className="text-slate-800">
                {selectedReq.clinicName}
              </strong>
              ?
            </p>
              ...
              {modalAction === "approve" ? (
                ...
                >
                  {actionLoading ? (
                    ...
                    <>
                      <CheckIcon className="h-5 w-5" /> Authorize
                    </>
                  )}
                </button>
              ) : (
