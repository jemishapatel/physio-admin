...
const ClinicJoinRequests: React.FC = () => {
  ...
  const fetchRequests = async (page = 1) => {
    ...
    try {
      ...
      const response = await userAPI.getClinicJoinRequests(params);
      const data = response.data.data;
      const user = data.user;

      const mapped: JoinRequest[] = (user?.clinics || []).map((c: any) => ({
        userId: user.id,
        ...
        clinicId: c.clinicId?._id || c._id,
        clinicName: c.clinicId?.clinicName || c.clinicName || '—',
        clinicEmail: c.clinicId?.email || c.email,
        clinicMobile: c.clinicId?.mobileNumber,
        address: c.clinicId?.address || c.address,
        businessLink: c.clinicId?.businessLink || c.businessLink,
        isClinicApproved: c.clinicId?.isApproved,
        isClinicActive: c.clinicId?.isActive,
        ownerId: c.clinicId?.ownerId,
        isApproved: c.isApproved,
        approvedBy: c.approvedBy || null,
        approvedAt: c.approvedAt || null,
        rejectionReason: c.rejectionReason || null,
        addedAt: c.addedAt,
        status: c.status || (c.isApproved === true ? 'approved' : 'pending'),
      }));

      setRequests(mapped);
      setSummary(data.summary || { total: 0, pending: 0, approved: 0 });
      if (data.pagination) {
        setPagination({
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-6xl mx-auto relative">
      ...
      <div className="border-b border-slate-100 pb-2">
        ...
        <p className="text-slate-500 mt-2.5 text-base font-medium">Manage and review doctor affiliation requests linking to your clinic</p>
      </div>
...
export default ClinicJoinRequests;
